...
/*   Updated: 2022/10/11 21:40:58 by vcedraz-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!line_afterbrk)
		...
		line.with_n = ft_strjoin(line_afterbrk, reading_function(fd));
	len = ft_strlen(line.with_n);
	if (len)
	{
		line_afterbrk = sbstr_aft_lnbrk(line.with_n, len + 1);
...
char	*sbstr_til_lnbrk(char *ln_with_n, size_t big_len)
{
	void	*n_address;
	...
	if (!n_address || n_address == ln_with_n + big_len)
		return (ln_with_n);
	return (ft_substr(ln_with_n, 0, n_address - (void *)ln_with_n + 1));
}
